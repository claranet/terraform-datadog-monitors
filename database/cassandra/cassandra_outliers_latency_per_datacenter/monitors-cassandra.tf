resource "datadog_monitor" "datadog_monitor_cassandra_outliers_latency" {
  ...
  name    = "${var.prefix_slug == "" ? "" : "[${var.prefix_slug}]"}[${var.environment}] DSE Cassandra datacenter:${var.cassandra_outliers_latency_datacenter_name} outliers_latency {{#is_alert}}{{{comparator}}} {{threshold}}% ({{value}}%){{/is_alert}}{{#is_warning}}{{{comparator}}} {{warn_threshold}}% ({{value}}%){{/is_warning}}"
  ...
  ${var.cassandra_outliers_latency_time_aggregator}(${var.cassandra_outliers_latency_timeframe}):
   outliers(avg:cassandra.latency.75th_percentile{datacenter:${var.cassandra_outliers_latency_datacenter_name}} by {host}, 'scaledMAD', 5, 90) > 0
EOQ
