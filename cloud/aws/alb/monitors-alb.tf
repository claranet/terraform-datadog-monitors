resource "datadog_monitor" "ALB_no_healthy_instances_warning" {
  ...
  query = <<EOQ
    ...
    ) * 100 < ${var.alb_no_healthy_instances_threshold_warning}
EOQ

  monitor_thresholds {
    critical = var.alb_no_healthy_instances_threshold_warning
  }
...
EOQ
  ...
  monitor_thresholds {
    critical = var.latency_threshold_warning
  }
...
resource "datadog_monitor" "ALB_httpcode_5xx_warning" {
  count   = var.httpcode_alb_5xx_enabled == "true" ? 1 : 0
  name    = "${var.prefix_slug == "" ? "" : "[${var.prefix_slug}]"}[${var.environment}] [{{loadbalancer.name}}] ALB HTTP code 5xx {{#is_alert}}{{{comparator}}} {{threshold}}% ({{value}}%){{/is_alert}}{{#is_warning}}{{{comparator}}} {{warn_threshold}}% ({{value}}%){{/is_warning}}"
  message = coalesce(var.httpcode_alb_5xx_message, var.message_warning)
  type    = "query alert"
  ...
  query = <<EOQ
    ${var.httpcode_alb_5xx_time_aggregator}(${var.httpcode_alb_5xx_timeframe}):
      ...
      * 100 > ${var.httpcode_alb_5xx_threshold_warning}
EOQ

  monitor_thresholds {
    critical = var.httpcode_alb_5xx_threshold_warning
  }
...
resource "datadog_monitor" "ALB_httpcode_5xx_critical" {
  count   = var.httpcode_alb_5xx_enabled == "true" ? 1 : 0
  name    = "${var.prefix_slug == "" ? "" : "[${var.prefix_slug}]"}[${var.environment}] [{{loadbalancer.name}}] ALB HTTP code 5xx {{#is_alert}}{{{comparator}}} {{threshold}}% ({{value}}%){{/is_alert}}{{#is_warning}}{{{comparator}}} {{warn_threshold}}% ({{value}}%){{/is_warning}}"
...
resource "datadog_monitor" "ALB_httpcode_4xx_warning" {
  count   = var.httpcode_alb_4xx_enabled == "true" ? 1 : 0
  name    = "${var.prefix_slug == "" ? "" : "[${var.prefix_slug}]"}[${var.environment}] [{{loadbalancer.name}}] ALB HTTP code 4xx {{#is_alert}}{{{comparator}}} {{threshold}}% ({{value}}%){{/is_alert}}{{#is_warning}}{{{comparator}}} {{warn_threshold}}% ({{value}}%){{/is_warning}}"
  message = coalesce(var.httpcode_alb_4xx_message, var.message_warning)
  type    = "query alert"
  ...
  query = <<EOQ
    ${var.httpcode_alb_4xx_time_aggregator}(${var.httpcode_alb_4xx_timeframe}):
      ...
      default(avg:aws.applicationelb.request_count${module.filter-tags.query_alert} by {region,loadbalancer}.as_rate() + ${var.artificial_requests_count}, 1))
      * 100 > ${var.httpcode_alb_4xx_threshold_warning}
EOQ

  monitor_thresholds {
    critical = var.httpcode_alb_4xx_threshold_warning
  }
...
resource "datadog_monitor" "ALB_httpcode_4xx_critical" {
  count   = var.httpcode_alb_4xx_enabled == "true" ? 1 : 0
  name    = "${var.prefix_slug == "" ? "" : "[${var.prefix_slug}]"}[${var.environment}] [{{loadbalancer.name}}] ALB HTTP code 4xx {{#is_alert}}{{{comparator}}} {{threshold}}% ({{value}}%){{/is_alert}}{{#is_warning}}{{{comparator}}} {{warn_threshold}}% ({{value}}%){{/is_warning}}"
...
resource "datadog_monitor" "ALB_httpcode_target_5xx_warning" {
  ...
  query = <<EOQ
    ${var.httpcode_target_5xx_time_aggregator}(${var.httpcode_target_5xx_timeframe}):
      default(avg:aws.applicationelb.httpcode_target_5xx${module.filter-tags.query_alert} by {region,loadbalancer,targetgroup,service}.as_rate(), 0) / (
      default(avg:aws.applicationelb.request_count${module.filter-tags.query_alert} by {region,loadbalancer,targetgroup,service}.as_rate() + ${var.artificial_requests_count}, 1))
      * 100 > ${var.httpcode_target_5xx_threshold_warning}
EOQ

  monitor_thresholds {
    critical = var.httpcode_target_5xx_threshold_warning
  }
...
resource "datadog_monitor" "ALB_httpcode_target_4xx_warning" {
  ...
  query = <<EOQ
    ${var.httpcode_target_4xx_time_aggregator}(${var.httpcode_target_4xx_timeframe}):
      default(avg:aws.applicationelb.httpcode_target_4xx${module.filter-tags.query_alert} by {region,loadbalancer,targetgroup,service}.as_rate(), 0) / (
      default(avg:aws.applicationelb.request_count${module.filter-tags.query_alert} by {region,loadbalancer,targetgroup,service}.as_rate() + ${var.artificial_requests_count}, 1))
      * 100 > ${var.httpcode_target_4xx_threshold_warning}
EOQ

  monitor_thresholds {
    critical = var.httpcode_target_4xx_threshold_warning
  }
